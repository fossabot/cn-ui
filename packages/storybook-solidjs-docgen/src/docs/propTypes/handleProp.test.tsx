...
function extractPropDef(component: Component, rawDefaultProp?: any): PropDef {
	return enhancePropTypesProp(
		extractComponentProps(component, DOCGEN_SECTION)[0],
		rawDefaultProp,
	);
}
...
describe("enhancePropTypesProp", () => {
		...
		describe("custom", () => {
			describe("when raw value is available", () => {
				...
				it("should support short object", () => {
					...
					expect(type.summary.replace(/\s/g, "")).toBe(
						expectedSummary.replace(/\s/g, ""),
					);
					expect(type.detail).toBeUndefined();
				});
            ...
            value4: string
					...
					expect(type.detail?.replace(/\s/g, "")).toBe(
						expectedDetail.replace(/\s/g, ""),
					);
				});
	...
	describe("defaultValue", () => {
		...
		it("should support short object", () => {
			const component = createTestComponent(
				createDefaultValue("{ foo: 'foo', bar: 'bar' }"),
			);
			...
			expect(defaultValue?.summary.replace(/\s/g, "")).toBe(
				expectedSummary.replace(/\s/g, ""),
			);
        ...
        another: 'another'
			...
			expect(defaultValue?.detail?.replace(/\s/g, "")).toBe(
				expectedDetail.replace(/\s/g, ""),
			);
		});
		...
		it("should support short element", () => {
			const component = createTestComponent(
				createDefaultValue("<div>Hey!</div>"),
			);
		...
		it("should support element with props", () => {
			const component = createTestComponent(
				createDefaultValue('<Component className="toto" />'),
			);
