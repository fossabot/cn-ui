...
function generateTypeFromString(
	value: string,
...
function generateUnion(
	...
	if (Array.isArray(type.value)) {
		...
		return createTypeDef({
			...
			compact: values.compact.every((x: string) => x != null)
				? values.compact.join(" | ")
				: null,
...
function generateArray(
	...
	const { name, short, compact, full, inferredType } = generateType(
		type.value,
		extractedProp,
	);
	...
	try {
		switch (type.name) {
			...
			case PropTypesType.ARRAYOF: {
				...
				return full
					? createSummaryValue(short, full)
					: createSummaryValue(short);
			}
			case PropTypesType.FUNC: {
