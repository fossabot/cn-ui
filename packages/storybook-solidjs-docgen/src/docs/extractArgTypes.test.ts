...
describe("react component properties", () => {
	...
	const fixturesDir = path.resolve(
		__dirname,
		"../../template/stories/docgen-components",
	);
	fs.readdirSync(fixturesDir, { withFileTypes: true }).forEach((testEntry) => {
		if (testEntry.isDirectory()) {
			...
			const testFile = fs
				.readdirSync(testDir)
				.find((fileName) => inputRegExp.test(fileName));
			if (testFile) {
				if (skippedTests.includes(testEntry.name)) {
					...
					it(`${testEntry.name}`, () => {
						...
						expect(docgenPretty).toMatchSpecificSnapshot(
							path.join(testDir, "docgen.snapshot"),
						);
						...
						expect(properties).toMatchSpecificSnapshot(
							path.join(testDir, "properties.snapshot"),
						);
						...
						expect(rows).toMatchSpecificSnapshot(
