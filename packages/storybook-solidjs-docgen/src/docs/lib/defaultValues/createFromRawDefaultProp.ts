...
const functionResolver: TypeResolver = (rawDefaultProp, propDef) => {
	let isElement = false;
	let inspectionResult: ReturnType<typeof inspectValue>;
	...
	if (isFunction(rawDefaultProp.render)) {
		...
		isFunction(rawDefaultProp.prototype.render)
	) {
		isElement = true;
	} else {
		...
		try {
			...
			const { hasParams, params } =
				inspectionResult.inferredType as InspectionFunction;
			if (hasParams) {
...
export function createTypeResolvers(
	customResolvers: Partial<TypeResolvers> = {},
): TypeResolvers {
	return {
