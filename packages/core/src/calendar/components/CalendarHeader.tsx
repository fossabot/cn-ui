...
export function CalendarHeader() {
    ...
    return (
        <div class="flex  py-2 border-b gap-1">
            <Icon onclick={() => calendarSystem.targetDate((i) => i.add(isYearView() ? -12 : -1, 'year'))} class={iconClass}>
            ...
            <div class="flex-1 text-center">
                <Show
                    ...
                    fallback={
                        <>
                            <span class="hover:text-primary-400 px-1 cursor-pointer" onclick={() => calendarSystem.calendarShowingType('year')}>
                                {calendarSystem.targetDate().year()}
                            </span>
                            <Show when={calendarSystem.calendarShowingType() === 'day'}>
                                <span class="hover:text-primary-400 px-1 cursor-pointer" onclick={() => calendarSystem.calendarShowingType('month')}>
            ...
            <Icon onclick={() => calendarSystem.targetDate((i) => i.add(isYearView() ? 12 : 1, 'year'))} class={iconClass + ' rotate-180'}>
                <AiOutlineDoubleLeft></AiOutlineDoubleLeft>
            </Icon>
        </div>
    )
