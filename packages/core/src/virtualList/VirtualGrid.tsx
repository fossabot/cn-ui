...
export function VirtualGrid<T>(props: VirtualGridProps<T>) {
	...
	const colVirtualizer = createVirtualizer({
		...
		get overscan() {
			return (
				props.overscan ?? Math.min(20, Math.floor(Math.sqrt(props.each.length)))
			);
		},
	});
	const { height, width } = useAutoResize(
		() => tableContainerRef()?.parentElement!,
	);
	const CoreList = (
			...
			{(virtualRow) => {
				...
				return (
					<Show when={row}>
						<div
							class={classNames(
								"cn-virtual-list-item absolute w-full",
								itemClass(),
							)}
