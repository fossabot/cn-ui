...
export const GroupList = OriginComponent<GroupListProps>((props) => {
	...
	const innerContent = (item: CommonGroupListConfig) => {
		const isHeader = computed(
			() => isGroupListConfigWithOptions(item) && props.unfold,
		);
		return (
			<>
				<div
					...
					<div class={classNames("w-full px-1 py-1")}>
						<Show when={!isHeader()}>
							<Icon class="w-4 h-4 mr-2">
								{ensureFunctionResult(item.icon)}
							</Icon>
						</Show>
	...
	return (
		<Flex
			vertical
			justify="start"
			class={classNames(!props.pure && "p-2 shadow-3 rounded-md")}
		>
			<For each={flatOptions()} fallback={VoidSlot()}>
				{(item) => {
					if (!props.unfold && "options" in item) {
						const open = computed(() =>
							ensureFunctionResult(props.open, [item]),
						);
						return (
							<Popover
