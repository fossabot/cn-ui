...
export function FormCore<T, D>(props: FormCoreProps<T, D>) {
	const accessorKey = createMemo(() =>
		getKeyFromRootColumnDef(
			props.config,
			props.originData as T,
			props.index ?? 0,
		),
	);
	...
	const isRequired = createMemo(
		() =>
			...
			ensureArrayReturn(props.config.rules).some((i) => i?.required),
	);
	...
	const errorMessage = createMemo(() => {
		...
		return formCtx
			?.validResult?.()
			?.fields[accessorKey()]?.map((i) => i.message as string);
	});
	return (
		<Col
		...
		>
			<Show when={props.showLabel}>
				<label
					...
					class={classNames(
						"flex-none w-full",
						props.wrap ? "text-left" : "text-right",
					)}
