...
export const useStaticTableDefine = <T>(props: MagicTableProps<T>) => {
	const [rowSelection, onRowSelectionChange] =
		createStateLinker<RowSelectionState>({});
	const [sorting, onSortingChange] = createStateLinker<SortingState>([]);
	const [columnVisibility, onColumnVisibilityChange] =
		createStateLinker<VisibilityState>({});
	const [columnSizing, onColumnSizingChange] =
		createStateLinker<ColumnSizingState>({});
	const [expanded, onExpandedChange] = createStateLinker<ExpandedState>({});
	const [columnOrder, onColumnOrderChange] =
		createStateLinker<ColumnOrderState>([]);
	...
	const table = createSolidTable<T>({
		...
		getSubRows: (row, index) =>
			props.getSubRows?.(row, index) ?? (row as any).subRows,
