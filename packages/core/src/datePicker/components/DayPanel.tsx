            ...
            <DatePicker.Table class="w-full cn-date-panel">
                <DatePicker.TableHead>
                    <DatePicker.TableRow>
                        {api().weekDays.map((weekDay) => (
                            <DatePicker.TableHeader class="font-normal py-1">
                                {weekDay.narrow}
                ...
                <DatePicker.TableBody>
                    {api().weeks.map((week) => (
                        <DatePicker.TableRow>
                            {week.map((day) => {
                                const isEdge = createMemo(() =>
                                    api().value.some((i) => i.toString() === day.toString()),
                                );
                                return (
                                    <DatePicker.TableCell
                                        class={classNames(isEdge() ? "px-0" : "px-0", "w-12 h-12")}
                                        value={day}
                                    >
                                        {/* 样式压不住，所以采用类名 */}
