...
import { type Atom, reflect } from "..";
import { SetAtom, type SetAtomType } from "../SetAtom";
...
describe("SetAtom", () => {
	...
	describe("intersection", () => {
		...
		it("should return a new set with elements that are common to both sets when by function is provided", () => {
			...
			expect(setAtom.intersection(setOrArray, Math.floor)).toEqual(
				new Set([2]),
			);
		});
	});
	...
	describe("union", () => {
		...
		it("should return a new set with elements from both sets when by function is provided", () => {
			...
			expect(setAtom.union(setOrArray, Math.floor)).toEqual(
				new Set([1, 2, 4.5, 5.5]),
			);
		});
	});
