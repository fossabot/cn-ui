...
export interface ResourceAtom<T, Params = unknown>
	extends ResourceBase<T, Params>,
		Atom<T> {}
...
export const resource = <T, Params = unknown>(
	...
	const refetch = async (
		val?: Params,
		{ warn = true, cancelCallback } = defaultRefetch,
	) => {
		// 上一次请求尚未结束
		if (!isReady()) {
			if (cancelCallback) {
	...
	deps?.length &&
		useEffectWithoutFirst(() => refetch(undefined, defaultRefetch), deps);
