...
export function useSelect<T>(
	...
	const optionsIdMap = computed(
		() => new Map<string, T>(options().map((i) => [getId(i), i])),
	);
	...
	const changeSelected = (id: string, state?: boolean) => {
		...
		if (state === true) {
			updateMapAtom(selectedMap, (map) => {
				...
				if (!item)
					throw new Error(
						"useSelect | changeSelected error: id " +
							id +
	...
	const multiSelectedState = computed(() => {
		...
		const isNone =
			(!hasOneSelected && hasOneUnSelected) || optionsIdMap().size === 0;
