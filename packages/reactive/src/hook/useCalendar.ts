			...
			switch (mode()) {
				...
				case "multiple":
					return state
						? selectedDate((i) => [...i, d])
						: selectedDate((i) =>
								i.filter((selected) => !d.isSame(selected, unit())),
							);
				case "range":
					return state
						? selectedDate((i) => {
...
export const useDateCalendar = (
	targetDate: Atom<Dayjs>,
	config: Accessor<DateCalendarConfig>,
) => {
	const allDateInMonth = createMemo(() => {
		...
		return {
			...
			paddingStart: getDaysInRange(
				firstDateOfCalendar,
				firstDateOfMonth.add(-1, "d"),
			),
			paddingEnd: getDaysInRange(
